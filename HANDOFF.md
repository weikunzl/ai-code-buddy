...
1. Continue with live BLE simulator verification on the connected StickS3. The next useful command path is `tools/session_bridge.py --transport ble --simulate` from a local Python environment with `bleak` available, then record observed device behavior in `PROGRESS.md`.
