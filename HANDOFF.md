...
1. Continue with interactive BLE verification on the connected StickS3. The next useful check is a long-running `tools/session_bridge.py --transport ble --simulate` session and explicit confirmation that `A` approve / `B` deny-or-cycle commands are flowing back as expected.
