...
class EnsureBridgeTests(unittest.TestCase):
    def test_bridge_ports_from_url(self):
        ...
        self.assertEqual(ws, 19877)

    def test_is_local_bridge(self):
        self.assertTrue(ensure.is_local_bridge("http://127.0.0.1:19876"))
        self.assertFalse(ensure.is_local_bridge("http://192.168.0.1:19876"))
    ...
    def test_ensure_starts_when_down(self, _enabled, _running, start, _wait):
        ok = ensure.ensure_bridge_running("http://127.0.0.1:19876")
        self.assertTrue(ok)
    ...
    @mock.patch.object(ensure, "start_bridge_background")
    @mock.patch.object(ensure, "bridge_is_running", return_value=False)
    @mock.patch.object(ensure, "restart_in_progress", return_value=True)
    def test_ensure_waits_during_restart(self, _restart, _running, start, _wait):
        ok = ensure.ensure_bridge_running("http://127.0.0.1:19876")
        self.assertTrue(ok)
        start.assert_not_called()
